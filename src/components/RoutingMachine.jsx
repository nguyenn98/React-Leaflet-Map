...
const RoutingMachine = ({ from, to, mode, onRouteInfo }) => {
  ...
  useEffect(() => {
    ...
    // ⚡ Cấu hình router OSRM với geojson
    const control = L.Routing.control({
      waypoints: [L.latLng(from[0], from[1]), L.latLng(to[0], to[1])],
      ...
      createMarker: (i, waypoint) => {
        ...
        return L.marker(waypoint.latLng, {
          icon: L.divIcon({
            className: "",
            html: isStart
              ? `<div style="font-size: 22px; color: #1976d2;">●</div>`
              : `<div style="font-size: 22px; color: #ff6600;">■</div>`,
            iconSize: [24, 24],
            iconAnchor: [12, 24],
          }),
          draggable: true,
        ...
        patterns: [
          {
            ...
            symbol: L.Symbol.arrowHead({
              pixelSize: 10,
              polygon: false,
              pathOptions: { color: "#ff6600", weight: 2 },
            }),
          },
        ],
      }).addTo(map);
